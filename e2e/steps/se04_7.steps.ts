...
When('I close the filters panel', () => {
  cy.get('.filters-trigger').then(($host) => {
    if ($host.hasClass('open')) cy.get('body').click('topLeft');
  });
});
...
When('I add the first item to the cart', () => {
  cy.getMenuCards().first().within(() => {
    cy.contains('app-revolving-button', 'Add to Cart').click({ force: true });
  });
});

When('I remove the first item from the cart', () => {
  cy.getMenuCards().first().within(() => {
    cy.contains('app-revolving-button', 'Remove from Cart').click({ force: true });
  });
});
...
Then('the cart badge bounces briefly', () => {
  ...
  cy.tick(700);
  cy.get('.cart-badge').should('not.have.class', 'bounce');
});
